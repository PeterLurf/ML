...
params = torch.nn.Parameter(torch.tensor([0.1, 0.1]))  # initial guess (radians)
...
for it in range(num_iterations):
    ...
    if it % 50 == 0:
        print(f"Iter {it:03d}: Loss = {loss.item():.6f}, "
              f"theta1 = {params.data[0].item():.4f}, theta2 = {params.data[1].item():.4f}")

print("\nOptimization finished.")
print(f"Final initial angles: theta1 = {params.data[0].item():.6f} rad, "
      f"theta2 = {params.data[1].item():.6f} rad")

#############################################
# 6. Simulation and Plots of the Optimized System
#############################################
# Run a final simulation with the optimized initial angles.
full_states = simulate_double_pendulum_rk4(params.data[0], params.data[1], num_steps, dt)
